...
- (void)addFavorite: (UITapGestureRecognizer *)recognizer {
    if (alreadyFav == false) {
        NSString *url = [NSString stringWithFormat: @"http://192.168.1.79:8000/api/store/%@/%@", [self.user objectForKey:@"email"], [[self.selectedStore objectAtIndex: (int)self.posSelectedStore] objectForKey:@"_id"]];
        ...
        NSString *url = [NSString stringWithFormat: @"http://192.168.1.79:8000/api/store/delete/%@/%@", [self.user objectForKey:@"email"], [[self.selectedStore objectAtIndex: (int)self.posSelectedStore] objectForKey:@"_id"]];
